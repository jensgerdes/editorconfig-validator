...
/**
 * Checks that a given Character stream contains only {@link eu.b1n4ry.editorconfig.codingstyle.LineEndingStyle#LF}
 * line endings.
 */
public class LfCheck implements LineEndingHelper {

	private static final Logger LOG = LoggerFactory.getLogger(LfCheck.class);
	private static final String ERROR = "File(%s) contains non-compliant line-endings (CR).";

	private static final char NON_COMPLIANT_CHARACTER = '\r';
	...
	public void readCharacter(char character) {
		...
		if (NON_COMPLIANT_CHARACTER == character) {
			LOG.debug("Found a CR character.");
